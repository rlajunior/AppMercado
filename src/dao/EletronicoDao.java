...
public class EletronicoDao {
	...
	public static Object obterlista() {
		...
		try {
			...
			while (rs.next()) {
				lista.add(new Eletronico(rs.getInt("idproduto"), rs.getString("nome"), rs.getFloat("peso"),
						rs.getDouble("valor"), rs.getString("local"), rs.getString("localretirada"),
						rs.getInt("anogarantia"), rs.getBoolean("possuigarantia")));
			}
	...
	public static boolean salvar(Eletronico eletronico) {
		try {
			...
			PreparedStatement ps = Conexao.obterConexao().prepareStatement("INSERT INTO eletronico "
					+ "(ideletronico, local, localretirada, anogarantia, possuigarantia) " + "VALUES " + "(?,?,?,?,?)");
